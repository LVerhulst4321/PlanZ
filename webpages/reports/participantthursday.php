<?php
$report = [];
$report['name'] = 'Participant Availability on Thursday';
$report['description'] = 'Participants who said they were available on Thursday. Ignore all NULL entries.';
...
SELECT
    ...
    ORDER BY
         ...
         CD.firstname;
EOD;
$report['queries']['thursdaytimes'] =<<<'EOD'
SELECT
        P.badgeid,
    ...
    WHERE
        P.interested=1 
        AND ADDTIME('$ConStartDatim$',PAT.starttime) < ADDDATE('$ConStartDatim$',1);
EOD;
$report['xsl'] =<<<'EOD'
...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="doc/query[@queryName='participants']/row">
        ...
        <tr>
            ...
            <td class="report">
                <xsl:choose>
                    <xsl:when test="/doc/query[@queryName='thursdaytimes']/row[@badgeid=$badgeid]">
                        <xsl:apply-templates select="/doc/query[@queryName='thursdaytimes']/row[@badgeid=$badgeid]" />
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="doc/query[@queryName='thursdaytimes']/row">
        <div>
