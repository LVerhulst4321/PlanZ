...
EOD;
    ...
    if (!$mayIEditAllRoles) {
        if (($paramArray['permissionRoles'] && count(array_diff($paramArray['permissionRoles'], $rolesIMayEditArr))) > 0) {
            $message_error .= "Server configuration error: You are attempting to add roles you do not have permission to add. Seek assistance.<br />\n";
            return false;
        }
    }
...
RenderXSLT('AddUser.xsl', $paramArray, $XMLDoc);
staff_footer();
?>
